...
public class Main implements RequestHandler<S3Event, String> {
    ...
    private static final String DESTINATION_BUCKET = "trusted-allset";
    ...
    public String handleRequest(S3Event s3Event, Context context) {
        ...
        try {
            ...
            // Conversão do JSON para uma lista de objetos Componente usando o Mapper
            Mapper mapper = new Mapper();
            List<Componente> componentes = mapper.map(s3InputStream);

            // Geração do arquivo CSV a partir da lista de Componente usando o CsvWriter
            CsvWriter csvWriter = new CsvWriter();
            ByteArrayOutputStream csvOutputStream = csvWriter.writeCsv(componentes);
